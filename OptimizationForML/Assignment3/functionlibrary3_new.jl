...
function prox_grad_dual_coord(w,Q,g,gamma,stop,oldsol) #nu kör vi den ett fixt antal gånger för att hitta en riktig lösning och sen en annan gång för att komma nära
    ...
    x_iterates = Vector[]
    while i < stop && norm(tmp1-oldsol) > 3.6e-12
        i += 1
    ...
    if i == stop
        print("\nProx_grad_dual_coord finished, not conv\n")
        print(string(norm(tmp1-oldsol)))
    end
    return tmp1,i,x_iterates
end
