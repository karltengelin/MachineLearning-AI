...
[wall_loc_m, wall_loc_n] = find(grid>0);
...
for action = 1 : 3 % Evaluate all the different actions (left, forward, right).
    ...
    %feature 1, distance to apple
    apple_dist_now = norm([apple_loc_m-next_head_loc(1),apple_loc_n-next_head_loc(2)],2);
    apple_dist_before = norm([apple_loc_m-prev_head_loc(1),apple_loc_n-prev_head_loc(2)],2);
    
    if apple_dist_before > apple_dist_now
        state_action_feats(1, action) = 1;
    else
        state_action_feats(1, action) = 0;
    end
    
    %feature 2, if next position is in wall or in self
    ...
    %feature 3, if next movement is into apple
...
%     feature 4, distance to wall or self
%     wall_dist_now = norm([wall_loc_m-next_head_loc(1),wall_loc_n-next_head_loc(2)],1);
%     wall_dist_before = norm([wall_loc_m-prev_head_loc(1),wall_loc_n-prev_head_loc(2)],1);
%     if wall_dist_now < wall_dist_before
...
%     
    ...
    %state_action_feats(2, action) = 1/crash;
