function cifar10_starter()
    ...
    %saker att testa: dropouts, olika sorters layers, convolution med
    %stigande antal filter, ändra learning rate, ändra stride i convolution
    % first step (with starter network) ändra runt i weight decay,
    % iterations etc (i training opts)
    %augmentera bilderna man skickar in för att få fler bilder att ta
    %sample på
    

    % argument=2 is how many 10000 images that are loaded. 20000 in this
    % example. Load as much as your RAM can handle.
    [x_train, y_train, x_test, y_test, classes] = load_cifar10(3);
    ...
    %net.layers{end+1} = struct('type', 'convolution',...
        %'params', struct('weights', 0.1*randn(3,3,16,16)/sqrt(5*5*16/2), 'biases', zeros(16,1)),...
        %'padding', [1 1]);
